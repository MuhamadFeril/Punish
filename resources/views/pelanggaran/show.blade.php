...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    @if($pelanggaran->bukti_pelanggaran)
        <div class="card">
            <img src="{{ asset('storage/' . $pelanggaran->bukti_pelanggaran) }}" alt="Bukti" class="w-full rounded-md">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-lg font-semibold">
                        <a href="{{ route('karyawan.show', $pelanggaran->karyawan->id) }}" class="text-blue-600 hover:underline">
            ...
            <div class="card border-2 border-red-300">
                ...
                <div class="space-y-2">
                    <div>
                        ...
                        <p class="text-lg font-semibold">{{ ucfirst($pelanggaran->sanksi->jenis_sanksi) }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Tanggal Sanksi</p>
                        <p class="text-lg font-semibold">{{ \Carbon\Carbon::parse($pelanggaran->sanksi->tanggal_sanksi)->format('d-m-Y') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Status</p>
                        <p class="inline-block px-3 py-1 rounded-full text-sm {{ $pelanggaran->sanksi->status === 'aktif' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                            {{ ucfirst($pelanggaran->sanksi->status) }}
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Keterangan</p>
                        <p class="text-gray-600">{{ $pelanggaran->sanksi->keterangan_sanksi }}</p>
                    </div>
                </div>
                <div class="mt-4">
                    <a href="{{ route('sanksi.show', $pelanggaran->sanksi->id) }}" class="btn btn-secondary">Lihat Detail Sanksi</a>
                </div>
            </div>
        @else
